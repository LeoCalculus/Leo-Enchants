"""
Generates a pure black Minecraft player skin with glowing red eyes
for the Shadow Clone entity.

Requirements: pip install Pillow

Output: src/main/resources/assets/leo_enchants/textures/entity/shadow_clone.png
"""

from PIL import Image, ImageDraw

def create_shadow_clone_skin():
    ...
    black = (5, 5, 5, 255)  # Slightly off-black for depth
    pure_black = (0, 0, 0, 255)
    
    # Glowing red for eyes
    red_glow = (255, 32, 32, 255)
    red_bright = (255, 80, 80, 255)
    
    # Fill the entire skin with black (all body parts)
    # This covers head, body, arms, legs in the skin layout
    ...
    # === HAT/OVERLAY LAYER (second layer, slightly transparent for glow effect) ===
    ...
    # Left eye: x=9-10, Right eye: x=13-14
    
    # Left eye (2 pixels wide)
    draw.rectangle([9, 11, 10, 12], fill=red_glow)
    
    # Right eye (2 pixels wide)  
    draw.rectangle([13, 11, 14, 12], fill=red_glow)
    
    # Add brighter center pixel to each eye for glow effect
    img.putpixel((9, 11), red_bright)
    img.putpixel((14, 11), red_bright)
    
    # Also add eyes to the hat/overlay layer for extra glow
    # Hat front is at 40-48 x, 8-16 y
    draw.rectangle([41, 11, 42, 12], fill=red_glow)
    draw.rectangle([45, 11, 46, 12], fill=red_glow)
    
    # Save the image
    output_path = "src/main/resources/assets/leo_enchants/textures/entity/shadow_clone.png"
    img.save(output_path, "PNG")
    print(f"Shadow clone skin saved to: {output_path}")
    print("Skin features:")
    print("  - Pure black body (slightly off-black #050505 for depth)")
    print("  - Glowing red eyes (#FF2020)")
